...
class LsTool:
    ...
    def execute(
        ...
        # 1. パスの解決とセキュリティチェック
        resolved_path = os.path.abspath(os.path.join(self.target_dir, dir_path))
        
        try:
            if os.path.commonpath([self.target_dir, resolved_path]) != self.target_dir:
                 return {"error": f"Access denied: {dir_path} is outside the target directory."}
        except ValueError:
