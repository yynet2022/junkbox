...
class ReadFileTool:
    ...
    def validate(self, file_path: str, offset: int = 0, limit: Optional[int] = None) -> Optional[str]:
        ...
        resolved_path = os.path.abspath(os.path.join(self.config.target_dir, file_path))
        ...
        try:
            ...
            if os.path.commonpath([self.config.target_dir, resolved_path]) != self.config.target_dir:
                 return f"Access denied: {file_path} is outside the target directory."
        except ValueError:
             # ドライブ文字が異なる場合など
    ...
    def execute(self, file_path: str, offset: int = 0, limit: Optional[int] = None):
        """ファイルを読み込みます"""
        # バリデーション実行
        ...
        try:
            ...
            if is_truncated:
                header = (
                    ...
                    f"Status: Showing lines {start_display}-{end_display} of {total_lines} total lines.\n"
                    f"Action: To read more, use offset={end_display} in the next call.\n"
